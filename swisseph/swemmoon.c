...
*
* This version of cmoon.c adjusts the ELP12000-85 analytical Lunar
...
*
*   M. Chapront-Touze' and J. Chapront, "ELP12000-85: a semi-analytical
...
int swi_moshmoon(double tjd, AS_BOOL do_save, double *xpmret, char *serr) {
     ...
     * Besides, this heLP1s to keep the program structure simpler
	 */
...
static void moon3() {
    ...
    moonpol[0] = LP1 + l + 1.0e-4 * moonpol[0];
...
int swi_mean_apog(double J, double *pol, char *serr) {
	...
	mean_elements();
    pol[0] = swi_mod2PI((LP1 - MP) * STR + PI);
#if 0
...
#if 0
	/* speed */
    mean_elements(T-PLAN_SPEED_INTV, &LP1, &MP, &NF, &M, &D);
    pol[3] = swi_mod2PI((LP1 - MP) * STR + PI);
	pol[4] = 0;
...
void swi_mean_lunar_elements(double tjd,
	...
	T2 = T*T;
	mean_elements();
    *node = swe_degnorm((LP1 - NF) * STR * RADTODEG);
    *peri = swe_degnorm((LP1 - MP) * STR * RADTODEG);
	T -= 1.0 / 36525;
	mean_elements();
    *dnode = swe_degnorm(*node - (LP1 - NF) * STR * RADTODEG);
	*dnode -= 360;
    *dperi = swe_degnorm(*peri - (LP1 - MP) * STR * RADTODEG);
}
...
static void mean_elements() {
    ...
    LP1 = mods3600( 1732564372.83264 * T +  785939.95571 );
    ...
    LP1 += (((((z[23]*T+z[22])*T + z[21])*T + z[20])*T + z[19])*T + z[18])*T2;
#else
	/* Mean distance of moon from its ascending node = F */
    ...
    /*LP1 = mods3600((1732564372.83264 - 6.784914260953e-01) * T +  785939.95571);*/
    LP1 = mods3600(1731456000.0 * fracT + 1108372.83264 * T - 6.784914260953e-01 * T +  785939.95571);
	/* Higher degree secular terms found by least squares fit */
	NF += ((z[2]*T + z[1])*T + z[0])*T2;
	MP += ((z[5]*T + z[4])*T + z[3])*T2;
	D  += ((z[8]*T + z[7])*T + z[6])*T2;
    LP1 += ((z[11]*T + z[10])*T + z[9])*T2;
#endif	/* ! MOSH_MOON_200 */
