...
static int print_line(int mode) {
	...
	int32 iflgret;
	for (sp = fmt; *sp != '\0'; sp++) {
		if (is_house && strchr("bBsSrRxXuUQnNfF+-*/=", *sp) != NULL)
			continue;
		if (sp != fmt)
			printf(gap);
		switch(*sp) {
		...
		case 's':
			if (*(sp+1) == 'S' || *(sp+1) == 's' || strpbrk(fmt, "XUxu") != NULL) {
				for (sp2 = fmt; *sp2 != '\0'; sp2++) {
					...
					case 'X':   /* speed! */
						if (is_label) {
							printf("speed_0%sspeed_1%sspeed_2",gap,gap);
							break;
						}
						if (*sp =='U')
...
static int do_eclipse_etc() {
	...
	if (smod & (SMOD_LUNAR | SMOD_SOLAR | SMOD_OCCULT))  {
		...
		if (smod & SMOD_ANNULAR)
			search_flag |= SE_ECL_ANNULAR | SE_ECL_PENUMBRAL;
		if (smod & SMOD_PARTIAL)
