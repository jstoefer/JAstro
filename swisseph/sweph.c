...
static const int pnoext2int[] = {
                                    SEI_SUN, SEI_MOON, SEI_MERCURY, SEI_VENUS, SEI_MARS, SEI_JUPITER, SEI_SATURN, SEI_URANUS, SEI_NEPTUNE, SEI_PLUTO, 0, 0, 0, 0, SEI_EARTH, SEI_CHIRON, SEI_PHOLUS, SEI_CERES, SEI_PALLAS, SEI_JUNO, SEI_VESTA,
                                };
...
do_fict_plan:
		...
		if (swi_osc_el_plan(tjd, pdp->x, ipl-SE_FICT_OFFSET, ipli, pedp->x, psdp->x, serr)
			        != OK)
			goto return_error;
			if (retc == ERR)
			goto return_error;
...
static const struct meff_ele FAR eff_arr[] = {
        ...
        **********************************************************/
        ...
        int swi_get_observer(double tjd, int32 iflag,
            ...
            double cosfi, sinfi, cc, ss, cos_l, sin_l, h;
            ...
            cos_l = cos((swed.topd.geolon + sidt) * DEGTORAD);
            sin_l = sin((swed.topd.geolon + sidt) * DEGTORAD);
            h = swed.topd.geoalt;
            xobs[0] = (re * cc + h) * cosfi * cos_l;
            xobs[1] = (re * cc + h) * cosfi * sin_l;
